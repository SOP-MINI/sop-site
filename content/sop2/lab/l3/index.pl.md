...
- Zmapować obszar pamiędzi dzielonej (o tym w drugiej części tego tutoriala).
...
- `off` - offset w pliku, który chcemy zmapować.
...
- `munmap` - zmapowaną pamięć trzeba kiedyś "odmapować" - służy do tego to polecenie. Patrz `man 3p munmap`.
...
Stworzony w ten sposób mutex będzie mógł być poprawnie współdzielony między procesami, to jednak nie koniec problemów. Wciąż bowiem obowiązuje nas podstawowe ograniczenie - mutex musi zostać odblokowany w tym samym wątku, który go zablokował. Stwarza to problem, gdy w wyniku błędu albo awarii proces zablokuje mutex i zakończy się przed jego odblokowanie. Mutex znajdzie się wtedy w niepoprawnym stanie. Aby zaradzić takim sytuacjom należy stworzyć tzw. "niezawodny mutex" (ang. "robust mutex").
