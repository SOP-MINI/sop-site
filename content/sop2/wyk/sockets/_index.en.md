...
- Sockets and UNIX file system: socket/file descriptors, read/write/close, error handling, socket API belongs to kernel,
  Unix domain (local) sockets - special files.
- Models of simple (request-reply) client-server communication using connection-oriented (TCP) and datagram (UDP)
  communication.
...
- Using host names: structure `hostent` and functions `gethostbyname()`, `gethostbyaddr()`; structure `addrinfo` and
...
- Data transfer via TCP connection: `read()`, `recv()`, `readv()`,`recvmsg`, `write()`, `send()`, `writev()`, `sendmsg`;
  `SIGPIPE` signal.
