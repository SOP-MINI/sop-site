...
- Gniazda a system plików UNIXa: deskryptory gniazd/plików, read/write/close, obsługa błędów, API gniazd częscią
  interfejsu jądra.
...
- Korzystanie z nazw stacji: struktura `hostent` i funkcje `gethostbyname()`, `gethostbyaddr()`; struktura `addrinfo` i
  funkcje: `getaddrinfo()`, `freeaddrinfo()`, `gai_strerror()`, `getnameinfo()`
...
- Przesyłanie danych przez połączenie TCP: `read()`, `recv()`, `readv()`,`recvmsg`, `write()`, `send()`, `writev()`,
  `sendmsg`, sygnał `SIGPIPE`.
...
2. Dokumentacja *The GNU C library*: rozdz.
